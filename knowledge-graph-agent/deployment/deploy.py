"""Deployment script for Knowledge Graph Agent using Robust Architecture."""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Deploy Knowledge Graph Agent to Vertex AI.")
    ...
    parser.add_argument("--gemini_app_project", type=str, default=None, help="Project ID where the Gemini App is located.")
...
def register_agent(remote_agent: object, project_id: str, app_id: str, app_project: str = None) -> None:
    ...
    target_project = app_project if app_project else project_id
    url = f"https://discoveryengine.googleapis.com/v1alpha/projects/{target_project}/locations/global/collections/default_collection/engines/{app_id}/assistants/default_assistant/agents"
    ...
    payload = {
        "displayName": f"{remote_agent.display_name}",
...
def create(args) -> None:
    ...
    remote_agent = agent_engines.create(
        ...
        requirements=[
            "google-adk==1.24.0",
            "google-cloud-aiplatform[agent-engines]==1.136.0",
            "google-genai==1.62.0",
            "google-auth",
            "requests",
            "python-dotenv",
    ...
    app_project = args.gemini_app_project or os.getenv("GEMINI_APP_PROJECT")
    
    register_agent(remote_agent, project_id, app_id, app_project)
